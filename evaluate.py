...
def get_eval_scores(res, evaluator, toker):
    ...
    try:
        ...
        is_long = res['output'].apply(lambda x: len(str(x).strip().split('\n')) > 2)
...
def main():
    ...
    for model_name in args.model_names:
        ...
        print(args.generation_output_path)

        if args.original_traindataset:
            fname += f"_original_traindataset"
            args.model_name += f"_original_traindataset"

        lang_suffix = ""
        if args.dataset_language:
            if args.dataset_language != "english":
                fname += f"_{args.dataset_language}"
                args.model_name += f"_{args.dataset_language}"
                args.generation_output_path += f"_{args.dataset_language}"
                lang_suffix = args.dataset_language
        elif args.multijail_language:
            # fname += f"_{args.multijail_language}"
            # args.model_name += f"_{args.multijail_language}"
            args.generation_output_path += f"_{args.multijail_language}"
        ...
        if args.use_harmless:
            ...
            data_path = "./data"
            generation_output_path = args.generation_output_path + f"_multilingual_mset{args.mset}"
            print(generation_output_path)
            output_path = args.output_path + f"_multilingual_mset{args.mset}"
        elif args.use_paralled500:
            data_path = "./data"
            generation_output_path = args.generation_output_path + f"_use_paralled500"
            # print(generation_output_path)
            output_path = args.output_path + f"_use_paralled500"
        elif args.use_paralled_multilingual:
            data_path = "./data"
            generation_output_path = args.generation_output_path + "_use_paralled_multilingual_center"
            output_path = args.output_path + "_use_paralled_multilingual_center"
        else:
            data_path = './data'
        ...
        if args.prompt_inject_jailbreak != "none":
            output_path += f"_{args.prompt_inject_jailbreak}_attack"
            generation_output_path +=  f"_{args.prompt_inject_jailbreak}_attack"

        os.makedirs(f"{output_path}/{args.config}", exist_ok=True)

        if args.with_contrast:
            fname += "_with_contrast"
            args.model_name += "_with_contrast"
        ...
        if args.use_malicious:
            ...
            fname += f"_{lang_suffix}_multijail_{args.multijail_language}"
            args.model_name += f"_{lang_suffix}_multijail_{args.multijail_language}"
            file_path = f"{data_path}/MultiJail.csv"
            data = pd.read_csv(file_path)
            prompts = data[args.multijail_language].tolist()
        elif args.use_gcg:
            fname += "_gcg"
